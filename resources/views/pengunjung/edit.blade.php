...
@section('title', 'Edit Pengunjung - Rudenim Surabaya')

@section('main-content')
<header class="bg-white border-b border-gray-100 px-8 py-5 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-[#001845]">Edit Data Kunjungan</h1>
        <p class="text-xs text-gray-400 mt-0.5">Ubah rincian informasi data kunjungan masyarakat.</p>
    </div>
    <a href="{{ route('visitors.index') }}" class="flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-gray-700 bg-gray-50 hover:bg-gray-100 px-4 py-2 rounded-xl transition-colors">
...
<div class="p-8 max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <form action="{{ route('visitors.update', $pengunjung->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="nik" value="{{ old('nik', $pengunjung->nik) }}" required maxlength="16"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="telepon" value="{{ old('telepon', $pengunjung->telepon) }}" required
            ...
            <div class="space-y-1.5">
                ...
                <select name="deteni_id" required class="w-full px-4 py-2.5 border @error('deteni_id') border-red-400 @else border-gray-200 @enderror rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                    @foreach($detensi as $deteni)
                        <option value="{{ $deteni->id }}" {{ old('deteni_id', $pengunjung->deteni_id) == $deteni->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="datetime-local" name="tanggal_kunjungan" value="{{ old('tanggal_kunjungan', \Carbon\Carbon::parse($pengunjung->tanggal_kunjungan)->format('Y-m-d\TH:i')) }}" required
                ...
                {{-- Status Dropdown Pengunjung Edit --}}
                <div class="space-y-1.5">
                    <label class="text-sm font-semibold text-gray-600">Status Persetujuan</label>
                    <select name="status" required class="w-full px-4 py-2.5 border @error('status') border-red-400 @else border-gray-200 @enderror rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                        <option value="Pending" {{ old('status', $pengunjung->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                        <option value="Hadir" {{ old('status', $pengunjung->status) == 'Hadir' ? 'selected' : '' }}>Hadir</option>
                        <option value="Ditolak" {{ old('status', $pengunjung->status) == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                    </select>
            ...
            <div class="pt-4 flex justify-end gap-3">
                <a href="{{ route('visitors.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-gray-500 bg-gray-50 hover:bg-gray-100 text-center transition-colors">Batal</a>
                <button type="submit" class="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-sm shadow-blue-100">Perbarui Pengunjung</button>
            </div>
        </form>
    </div>
</div>
